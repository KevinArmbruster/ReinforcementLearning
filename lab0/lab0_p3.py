...
for episode in range(5):
    ...
    while not done:
        ...
        if len(replay_buffer) >= batch_size:
            ...
            states = np.array(list(zip(*batch))[0])
            actions = torch.tensor(list(zip(*batch))[1])
            actions = torch.reshape(actions, (-1, 1))

            state_tensors = torch.tensor(states, requires_grad=False)
            action_tensors = nn.forward(state_tensors)

            z = torch.gather(action_tensors, 0, actions)
